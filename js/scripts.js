$(document).ready(function () {
   // Flower container where flowers will appear
    ...
    // Create a CSS selector string by iterating over animationClasses
    var selector = '';
    for (var i = 0; i < animationClasses.length; i++) {
        if (i > 0) {
            selector += ', ';
        }
        selector += '.' + animationClasses[i];
    }
    ...
    function handleScroll() {
        for (var i = 0; i < animatedElements.length; i++) {
            ...
            // Add the 'active' class only when the element becomes visible
            if (isVisible) {
    ...
    var targetDate = new Date("2025-01-05T02:00:00");
    ...
    setInterval(function () {
        ...
        if (timeDifference > 0) {
            ...
            $("#countdownTimer").text("Đám Cưới Đã Diễn ra!");
        }
    }, 1000);

    var currentDate = new Date("2025-01-05");

// The date to mark (January 5th, 2025)
var markedDate = new Date("2025-01-05");
...
function renderCalendar(date) {
    ...
    var monthYearElement = $("#monthYear");

    // Set month and year
    monthYearElement.text(
    ...
    // Fill days before the first day
    for (var i = 0; i < firstDay; i++) {
        daysElement.append('<div class="empty"></div>');
    }
    ...
    for (var day = 1; day <= lastDate; day++) {
        ...
        var isMarkedDate =
            ...
            month === markedDate.getMonth() &&
            year === markedDate.getFullYear();
...
renderCalendar(currentDate);
    ...
    $('.flexslider').flexslider({
        animation: "slide",  // Slide animation for the slider
        controlNav: false,   // Disable navigation dots
        directionNav: true,  // Enable previous/next arrows
        slideshow: true,     // Enable auto slideshow
        slideshowSpeed: 3000, // Delay between slides
      });
